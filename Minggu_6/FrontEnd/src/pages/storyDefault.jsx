        ...
        <>
            <div className="w-full h-full pt-12 px-16 flex justify-center">
                <div className="w-4/6 grid grid-cols-3 gap-x-12 gap-y-6">
                    {data.map(item => (
                        <Link key={item.id} className="w-full h-60 border border-black rounded-xl" to={`${item.id}/overview`}>
                    ...
                    <Form action="/home/stories" method="POST">
                        <button type="submit" className="w-full h-60 border border-black rounded-xl flex flex-col justify-center items-center gap-y-3 bg-gray-100">
